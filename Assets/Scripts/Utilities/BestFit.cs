...
public static class BestFitExtensions {
	...
	public static void BestFit(this GameObject[] gameObjects)
	{
		...
		foreach (var go in gameObjects) {
			...
			foreach (var text in textObj)
			{
				...
				var newSize = dropdown ? GetBestFitSize(text, mono, dropdown.options.Select(o => o.text).ToList()) : GetBestFitSize(text, mono);
	...
	private static int GetBestFitSize(Text text, BestFit mono, List<string> newStrings = null)
	{
		var smallestFontSize = 0;
